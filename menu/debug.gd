...
func _process(_delta):
	if Input.is_action_just_pressed("debug"):
		visible = !visible
	if visible:
		var noise = TerrainGenerator.get_noise()
		text = "Position: " + _vector_to_string_appropriate_digits(player.transform.origin)
		text += "\nChunk: " + _vector_to_string_appropriate_digits(player.chunk_pos())
		text += "\nNoise: " + str(8.0 * noise.get_noise_2d(player.transform.origin.x, player.transform.origin.z))
